> 内容来自: [Typescript 入门教程](https://ts.xcatliu.com/basics/declaration-files.html#declare-var) 并有所修改

<div style="color:red">很好</div>
...
`声明文件`中的`声明语句` 需要使用 `declare` 或 `export`开头, `type` 和 `interface` 不受此限制:

```typescript
// src/test.d.ts

enum Lang { // 报错: .d.ts 文件中的顶级声明必须以 "declare" 或 "export" 修饰符开头。ts(1046)
	en,
	zh
}
